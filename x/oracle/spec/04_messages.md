...
The `Operator` field contains the operator address of the validator (prefixed `blackvaloper-`). The `Delegate` field is the account address (prefixed `black-`) of the delegate account that will be submitting exchange rate related votes and prevotes on behalf of the `Operator`.
